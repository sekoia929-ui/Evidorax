...
export async function getPapers(projectId) {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      extractions_structured(*),
      verifications(accuracy_rate, verified_count, total_fields)
    `)
...
export async function saveCorrection(paperId, userId, fieldName, originalValue, correctedValue) {
  const { error } = await supabase
    .from('corrections')
    .insert({ paper_id: paperId, user_id: userId, field_name: fieldName, original_value: originalValue, corrected_value: correctedValue })

  if (error) throw error

  // Also update the structured extraction
  const { error: updateError } = await supabase
    .from('extractions_structured')
    .update({ [fieldName]: correctedValue, updated_at: new Date().toISOString() })
    .eq('paper_id', paperId)
...
export async function createProject(userId, name, description, topic) {
  const { data, error } = await supabase
    .from('projects')
    .insert({ user_id: userId, name, description, topic })
    .select()
    .single()
...
export async function checkPaperLimit(userId) {
  ...
  return {
    used: profile.papers_used,
    limit: profile.papers_limit,
    remaining: profile.papers_limit - profile.papers_used,
    canUpload: profile.papers_used < profile.papers_limit || profile.plan !== 'free'
  }
}
